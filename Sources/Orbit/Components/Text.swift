...
public struct Text: View, FormattedTextBuildable, PotentiallyEmptyView, CustomStringConvertible {
    ...
    public var body: some View {
        if isEmpty == false {
            text(environment: textRepresentableEnvironment)
    ...
    func text(environment: TextRepresentableEnvironment, isConcatenated: Bool = false) -> SwiftUI.Text {
...
extension Text: TextRepresentable {
    ...
    public func text(environment: TextRepresentableEnvironment) -> SwiftUI.Text? {
        isEmpty
            ? nil
            : text(environment: environment, isConcatenated: true)
    }
}
