...
public struct Text: View {
    ...
    var lineSpacingAdjusted: CGFloat {
        size.lineHeight - size.height + (lineSpacing ?? 0)
    }
}
...
public extension Text {
    ...
    enum Size: Equatable {
        ...
        /// Original rendered text height rounded to pixels before padding adjustments.
        public var height: CGFloat {
            round(value * Font.fontSizeToHeightRatio * UIScreen.main.scale) / UIScreen.main.scale
        }

        /// Vertical padding needed to match designated line height.
...
struct TextPreviews: PreviewProvider {
    ...
    static var lineSpacing: some View {
        HStack(alignment: .top, spacing: .xxxSmall) {
            ...
            Group {
                Text("Text\nmultiline\n<strong>formatted</strong>", size: .large, lineSpacing: .xxxSmall)
                Text("Text\nmultiline\nwith <applink1>links</applink1>", size: .large, lineSpacing: .xxxSmall)
            }
            .background(Color.redLightHover.opacity(0.7))
            .overlay(Separator(color: .redNormal, thickness: .hairline), alignment: .centerFirstTextBaseline)
    ...
    struct LineHeight: View {
        ...
        var body: some View {
            HStack(spacing: .xxSmall) {
                ...
                .fixedSize()
                .background(Color.redLightHover.frame(height: originalHeight))
                .overlay(Separator(color: .redNormal, thickness: .hairline), alignment: .centerLastTextBaseline)

                Text("\(size.value.formatted) / \(size.lineHeight.formatted)", size: .custom(6), weight: .bold)
                    .environment(\.sizeCategory, .large)
            }
            .padding(.trailing, .xxSmall)
            .border(Color.redNormal, width: .hairline)
            .measured()
            .padding(.trailing, 80 * sizeCategory.controlRatio)
            .overlay(
                SwiftUI.Text("Native")
        ...
        var originalHeight: CGFloat {
            size.height * sizeCategory.ratio
        }

        var linePadding: CGFloat {
            size.linePadding * sizeCategory.ratio
        }
    }
